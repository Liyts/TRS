...
public class Main {
    ...
    public void myFilter(BufferedImage img){
        ...
        for(Y=2;Y<height-2;Y++){
            for (X=2;X<width-2;X++){
                ...
                if (resault_R < 0) resault_R = 0;
                if (resault_R > 255) resault_R = 255;
                if (resault_G < 0) resault_G = 0;
                if (resault_G > 255) resault_G = 255;
                if (resault_B < 0) resault_B = 0;
                if (resault_B > 255) resault_B = 255;
    ...
    public  void readToArray(String path){ //метод обработки изображения и вызывающий метод модификации изображения
        ...
        try {
            ...
            expansionFile = path.substring(path.indexOf('.',path.lastIndexOf('/'))+1).toUpperCase();
            //realization(bufImg);
            Filter(bufImg);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void realization(BufferedImage inputImage){ //метод увеличения значения каждого пикселя на 5 едениц
        ...
        /*try {
            String fileName = "newFile"+expansionFile;
            ImageIO.write(outputImage, expansionFile, new File("nibody+20."+expansionFile.toLowerCase()));
        } catch (IOException e) {
            e.printStackTrace();
        }*/
